...
const resolvers: Record<string, ResolverFn> = {
  ...
  async create_task(raw) {
    ...
    return {
      ...
      scheduledDate: resolveDate(raw.scheduledDate as string),
      estimatedDurationMinutes: typeof raw.estimatedDurationMinutes === "number" ? raw.estimatedDurationMinutes : undefined,
      notes: raw.notes as string | undefined,
    };
  },
  ...
  async update_habit(raw) {
    ...
    return {
      ...
      endIdToAdd: await resolveEndName(raw.endNameToAdd as string),
      endIdToRemove: await resolveEndName(raw.endNameToRemove as string),
      endIdToMoveTo: await resolveEndName(raw.endNameToMoveTo as string),
